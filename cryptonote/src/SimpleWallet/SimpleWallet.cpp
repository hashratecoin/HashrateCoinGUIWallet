...
inline std::string interpret_rpc_response(bool ok, const std::string& status) {
  ...
  if (ok) {
    if (status == CORE_RPC_STATUS_BUSY) {
      err = "the hashratecoin daemon is busy. Please try later";
    } else if (status != CORE_RPC_STATUS_OK) {
      err = status;
    }
  } else {
    err = "possible lost connection to the hashratecoin daemon";
  }
  return err;
}
...
struct TransferCommand {
  ...
  bool parseArguments(LoggerRef& logger, const std::vector<std::string> &args) {
    ...
    try {
        ...
        if (arg.size() && arg[0] == '-') {
          ...
          if (!m_currency.parseAccountAddressString(arg, de.addr)) {
            ...
            if (CryptoNote::parsePaymentId(arg, paymentId)) {
              ...
              logger(ERROR, BRIGHT_RED) << "Wrong hashratecoin address: " << arg;
            }
      ...
      if (dsts.empty()) {
        logger(ERROR, BRIGHT_RED) << "At least one destination hashratecoin address is required";
        return false;
      }
...
std::string tryToOpenWalletOrLoadKeysOrThrow(LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password) {
  ...
  if (walletExists) {
    logger(INFO) << "Loading the hashratecoin wallet...";
    std::ifstream walletFile;
    walletFile.open(walletFileName, std::ios_base::binary | std::ios_base::in);
    if (walletFile.fail()) {
      throw std::runtime_error("Error opening the hashratecoin wallet file '" + walletFileName + "'");
    }
    ...
    if (initError) { //bad password, or legacy format
      if (keysExists) {
        ...
        if (initError) {
          throw std::runtime_error("Failed to load the hashratecoin wallet: " + initError.message());
        }

        logger(INFO) << "Storing the hashratecoin wallet...";

        try {
          CryptoNote::WalletHelper::storeWallet(*wallet, walletFileName);
        } catch (std::exception& e) {
          logger(ERROR, BRIGHT_RED) << "Failed to store the hashratecoin wallet: " << e.what();
          throw std::runtime_error("Error saving the hashratecoin wallet file '" + walletFileName + "'");
        }

        logger(INFO, BRIGHT_GREEN) << "Stored ok";
        return walletFileName;
      } else { // no keys, wallet error loading
        throw std::runtime_error("can't load the hashratecoin wallet file '" + walletFileName + "', check the password");
      }
    } else { //new wallet ok 
      return walletFileName;
    }
    ...
    throw std::runtime_error("The hashratecoin wallet file '" + walletFileName + "' is not found");
  }
}
...
simple_wallet::simple_wallet(System::Dispatcher& dispatcher, const CryptoNote::Currency& currency, Logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("export_keys", boost::bind(&simple_wallet::export_keys, this, _1), "Show the secret keys of the openned hashratecoin wallet");
  m_consoleHandler.setHandler("balance", boost::bind(&simple_wallet::show_balance, this, _1), "Show the current hashratecoin wallet balance");
  ...
  m_consoleHandler.setHandler("bc_height", boost::bind(&simple_wallet::show_blockchain_height, this, _1), "Show the hashratecoin blockchain height");
  ...
  m_consoleHandler.setHandler("address", boost::bind(&simple_wallet::print_address, this, _1), "Show the current hashratecoin wallet public address");
  m_consoleHandler.setHandler("save", boost::bind(&simple_wallet::save, this, _1), "Save the hashratecoin wallet synchronized data");
  ...
  m_consoleHandler.setHandler("exit", boost::bind(&simple_wallet::exit, this, _1), "Close the hashratecoin wallet");
}
//----------------------------------------------------------------------------------------------------
...
bool simple_wallet::init(const boost::program_options::variables_map& vm) {
  ...
  if (!m_daemon_address.empty() && (!m_daemon_host.empty() || 0 != m_daemon_port)) {
    fail_msg_writer() << "you can't specify a hashratecoin daemon host or port several times";
    return false;
  }
  ...
  if (m_generate_new.empty() && m_wallet_file_arg.empty()) {
    std::cout << "Hashratecoin CLI wallet\n\n"
			 ...
			 "||O |||Open a hashratecoin wallet file || \n"
    ...
    std::cout << "Specify a hashratecoin wallet file name (e.g., myhashratecoinwallet.wallet).\n";
    std::string userInput;
    do {
      std::cout << "hashratecoin wallet file name: ";
  ...
  if (!m_daemon_address.empty()) {
    if (!parseUrlAddress(m_daemon_address, m_daemon_host, m_daemon_port)) {
      fail_msg_writer() << "Failed to parse the hashratecoin daemon address: " << m_daemon_address;
      return false;
    }
  } else {
  ...
  if (command_line::has_arg(vm, arg_password)) {
    ...
    fail_msg_writer() << "Failed to read the hashratecoin wallet password";
    return false;
  }
  ...
  if (!m_generate_new.empty()) {
    ...
    if (!writeAddressFile(walletAddressFile, m_wallet->getAddress())) {
      logger(WARNING, BRIGHT_RED) << "Couldn't write the hashratecoin wallet address file: " + walletAddressFile;
    }
  } else if (!m_import_new.empty()) {
    ...
    try {
      ...
      fail_msg_writer() << "Failed to load the hashratecoin wallet: " << e.what();
      return false;
    }
...
bool simple_wallet::new_wallet(const std::string &wallet_file, const std::string& password) {
  ...
  try {
    ...
    if (initError) {
      fail_msg_writer() << "Failed to generate a new hashratecoin wallet: " << initError.message();
      return false;
    }
    ...
    try {
      ...
      fail_msg_writer() << "Failed to save the new hashratecoin wallet: " << e.what();
      throw;
    }
    ...
    logger(INFO, BRIGHT_GREEN) <<
      "\n\nGenerated a new hashratecoin wallet: \n" << m_wallet->getAddress() << "\n\n" <<
      "Secret view key: \n" << Common::podToHex(keys.viewSecretKey) << "\n\n\n";
  }
  catch (const std::exception& e) {
    fail_msg_writer() << "Failed to generate a new hashratecoin wallet: " << e.what();
    return false;
  }
...
bool simple_wallet::new_wallet(Crypto::SecretKey &secret_key, Crypto::SecretKey &view_key, const std::string &wallet_file, const std::string& password) {
  ...
  try {
    ...
    logger(INFO, BRIGHT_GREEN) <<
      "\n\nImported the hashratecoin wallet: \n" << m_wallet->getAddress() << std::endl;
  }
  catch (const std::exception& e) {
    fail_msg_writer() << "Failed to import the hashratecoin wallet: " << e.what();
    return false;
  }
...
bool simple_wallet::start_mining(const std::vector<std::string>& args) {
  ...
  try {
    ...
    if (err.empty())
      success_msg_writer() << "Mining started in the hashratecoin daemon";
    else
...
bool simple_wallet::stop_mining(const std::vector<std::string>& args)
{
  ...
  try {
    ...
    std::string err = interpret_rpc_response(true, res.status);
    if (err.empty())
      success_msg_writer() << "Mining stopped in the hashratecoin daemon";
    else
...
void simple_wallet::connectionStatusUpdated(bool connected) {
  if (connected) {
    logger(INFO, GREEN) << "Wallet connected to the hashratecoin daemon.";
  } else {
    printConnectionError();
  }
}
...
void simple_wallet::externalTransactionCreated(CryptoNote::TransactionId transactionId)  {
  ...
  if (txInfo.totalAmount >= 0) {
    logger(INFO, GREEN) <<
      ...
      ", received " << m_currency.formatAmount(txInfo.totalAmount) << " hashratecoin";
  } else {
    logger(INFO, MAGENTA) <<
      logPrefix.str() << " transaction " << Common::podToHex(txInfo.hash) <<
      ", spent " << m_currency.formatAmount(static_cast<uint64_t>(-txInfo.totalAmount)) << " hashratecoin";
  }
...
bool simple_wallet::show_balance(const std::vector<std::string>& args/* = std::vector<std::string>()*/) {
  success_msg_writer() << "\n\nAvailable balance: " << m_currency.formatAmount(m_wallet->actualBalance()) << " hashratecoin\n" <<
    "Locked amount: " << m_currency.formatAmount(m_wallet->pendingBalance()) << " hashratecoin\n";
...
bool simple_wallet::show_blockchain_height(const std::vector<std::string>& args) {
  try {
    ...
    fail_msg_writer() << "Failed to get hashratecoin blockchain height: " << e.what();
  }
...
bool simple_wallet::transfer(const std::vector<std::string> &args) {
  try {
    ...
    if (tx == WALLET_LEGACY_INVALID_TRANSACTION_ID) {
      fail_msg_writer() << "Can't send hashratecoin";
      return true;
    }
    ...
    success_msg_writer(true) << "hashratecoin successfully sent, transaction " << Common::podToHex(txInfo.hash);
...
void simple_wallet::printConnectionError() const {
  fail_msg_writer() << "The wallet failed to connect to the hashratecoin daemon (" << m_daemon_address << ").";
}
...
int main(int argc, char* argv[]) {
  ...
  if (command_line::has_arg(vm, Tools::wallet_rpc_server::arg_rpc_bind_port)) {
    ...
    if (!command_line::has_arg(vm, arg_wallet_file)) {
      logger(ERROR, BRIGHT_RED) << "hashratecoin wallet file not set.";
      return 1;
    }

    if (!command_line::has_arg(vm, arg_daemon_address)) {
      logger(ERROR, BRIGHT_RED) << "hashratecoin daemon address not set.";
      return 1;
    }

    if (!command_line::has_arg(vm, arg_password)) {
      logger(ERROR, BRIGHT_RED) << "hashratecoin wallet password not set.";
      return 1;
    }
    ...
    if (!daemon_address.empty()) {
      if (!parseUrlAddress(daemon_address, daemon_host, daemon_port)) {
        logger(ERROR, BRIGHT_RED) << "Failed to parse the hashratecoin daemon address: " << daemon_address;
        return 1;
      }
    }
    ...
    try  {
      ...
      logger(INFO) << "\n\nAvailable balance: " << currency.formatAmount(wallet->actualBalance()) << " hashratecoin\n" <<
      "Locked amount: " << currency.formatAmount(wallet->pendingBalance()) << " hashratecoin\n";

      logger(INFO, BRIGHT_GREEN) << "Loaded ok";
    } catch (const std::exception& e)  {
      logger(ERROR, BRIGHT_RED) << "The hashratecoin wallet initialize failed: " << e.what();
      return 1;
    }
    ...
    if (!wrpc.init(vm)) {
      logger(ERROR, BRIGHT_RED) << "Failed to initialize the hashratecoin wallet rpc server";
      return 1;
    }
    ...
    logger(INFO) << "Starting the hashratecoin wallet rpc server";
    wrpc.run();
    logger(INFO) << "Stopped the hashratecoin wallet rpc server";
    ...
    if (!wal.init(vm)) {
      logger(ERROR, BRIGHT_RED) << "Failed to initialize the hashratecoin wallet"; 
      return 1; 
    }
    ...
    if (!wal.deinit()) {
      logger(ERROR, BRIGHT_RED) << "Failed to close the hashratecoin wallet";
    } else {
      logger(INFO) << "The hashratecoin wallet closed";
    }
  }
  return 1;
