...
//
// hashratecoin is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// hashratecoin is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with hashratecoin.  If not, see <http://www.gnu.org/licenses/>.
...
const char DEFAULT_WALLET_FILE_NAME[] = "hashratecoin.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "159.89.136.161:9999";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("hashratecoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hashratecoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hashratecoinwallet.desktop");
  ...
  res = autorunSettings.contains("hashratecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("hashratecoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.hashratecoin.hashratecoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Hashratecoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("hashratecoinWallet", appPath);
    } else {
      autorunSettings.remove("hashratecoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\hashratecoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:hashratecoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\hashratecoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\hashratecoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("hashratecoinwallet.cfg"));
